...
| Production completion scheduling consumes Task028B outcomes without duplicating them | Task028E/F docs, `completion_monitor_production_schedule.py`; `6305f5e`, `cc30c99`, `e10d4c9` | Exact immutable plan, strict preflight, one call per invocation and terminal quiescence; two early GW4 plans remained inactive/stale, while a later plan completed and was owner-deactivated |
...
| CI executables and Python dependency resolution are exact while update discovery is automated | `.github/workflows/ci.yml`, `pyproject.toml`, `uv.lock`, `.github/dependabot.yml`; `4037102`, `7484484`, `624649e`, `274e5bb` | Actions use reviewed full-SHA pins; exact uv verifies and syncs one universal lock; Dependabot may propose individual action or Python updates but cannot authorize or auto-merge them |
| Task033 uncertainty kernels remain pure research components | Task033C1 implementation and architecture guard; `d3fb026` | No filesystem, network, production prediction or promotion authority; synthetic validation does not establish predictive benefit |
| Task033 synthetic evaluation fails closed before real execution | Task033C2 contracts, evaluator, inference and tests; `7874ded`, `6d695af` | Pair-specific membership, complete-family bounds and conditional selection are deterministic; C3 authorization/loading/publication and real evidence remain separate prerequisites |
...
| RFC decision | Status at `274e5bb` | Boundary / consequence |
|---|---|---|
